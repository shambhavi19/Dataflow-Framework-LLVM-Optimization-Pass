...
namespace {
  ...
  //Class Analysis contains Transfer function logic for available expressions
  class Analysis : public Dataflow {
    ...
    public:
      ...
      //Transfer function: Available expressions. Function takes current Basic block and input vector as parameters
      //OUT[BB] = fn(IN[BB])
      ...
      BBAttributes transferFn(BitVector in, BasicBlock* current)
      {
        ...
        for (BasicBlock::iterator i = current->begin(), e = current->end(); i!=e; ++i) {
          ...
          if (BinaryOperator * BI = dyn_cast<BinaryOperator>(I)) {
            //Expression for RHS of the binary operator
          ...
          //Kill set
          //Expressions with LHS operancds are killed
    ...
    private:
      ...
      virtual bool runOnFunction(Function& F) {
        ...
        //Available expressions : Meet operator = Intersection, Direction = Forward
      	Analysis analysis(domain.size(), domain, INTERSECTION, FORWARD, boundary, init);
        //Run dataflow analysis and store result 
	      DFAResult available_result = analysis.dataflowAnalysis(F);
  ...
  char AvailableExpressions::ID = 1;
